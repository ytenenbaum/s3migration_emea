...
def build_s3_download_bucket_command(destDir, s3Path):
    return ("/opt/splunk/migration_scripts/s3cmd-master/s3cmd", "sync",
...
def build_s3_upload_bucket_command(bucketPath, instanceGuid, indexName, dirType):
    ...
    destS3Uri = "s3://%s/%s/%s/%s/%s/" % (
            ...
            dirType,
            bucketName)
    return ("/opt/splunk/migration_scripts/s3cmd-master/s3cmd", "sync", bucketPath, destS3Uri)
def build_s3_upload_extradata_command(itemPath, instanceGuid):
    ...
    destS3Uri = "s3://%s/%s/%s/%s/" % (
            ...
            KEY_EXTRADATA,
            itemPath)
    return ("/opt/splunk/migration_scripts/s3cmd-master/s3cmd", "sync", itemPath, destS3Uri)
def run_s3_command(cmdPieces, upOrDown):
    ...
    for i in range(5):
        ...
        sys.stdout.write("%s\n    %s=%s\n    %s=%s\n    ..." % (
            ...
            upOrDown == MODE_UPLOAD and "src" or "dst", cmdPieces[-1],
            upOrDown == MODE_UPLOAD and "dst" or "src", cmdPieces[-2]))
        sys.stdout.flush()
        ...
        #env["LD_LIBRARY_PATH"] = ""
        env["PYTHON_PATH"] = "/opt/splunk/lib/python2.7/"
...
class CMD_ListAllSplunkBucketsFromS3(object):
    ...
    def run(self, args):
        ...
        # Main retrieve/parse loop (loop: AWS only enumerates 1000 items by default).
        while True:
            # throws on any non-200 response.
            resp = self.issue_s3_request(s3Bucket, s3Path, getOffset)
            # DEBUG: print resp
            getOffset = self.parseS3ListingXML(coll, checksumURLs, resp, s3Bucket, s3Path)
            if not getOffset:
                break

        outPath = "bucketsInS3.json"
        ...
        if not checksumURLs:
            ...
            for csumPath in checksumURLs:
                dlCmd = ["/opt/splunk/migration_scripts/s3cmd-master/s3cmd", "cp", csumPath, "."]
